            ...
            if ( iContent->playVideoFlag == true )
            {
                if ( iContent->playSubNode == true )
                {
                    ...
                    if ( iContent->state == 1 )
                    {
                        ...
                        subStagePos = iContent->categories[indexToDraw].stagePos ;
                    }
                }
                else
                {
...
void testApp::toggleIdle()
{
    ...
    cout << "isTouchIdle was : " <<isTouchIdle ;

    isTouchIdle = !isTouchIdle ;
    cout << " -- now is : "  <<isTouchIdle << endl ;
    if ( isTouchIdle )
    {
        for ( int i = 0 ; i < nItems ; i++ )
        {
...
void testApp::tuioCursorAdded(TuioCursor & tcur)
{
    ...
    float closestDistance = 40000 ;
    int closestIndex = -2 ;


    for ( int i = 0 ; i < nItems ; i++ )
    {
        float dist = ofDist(screenPos.x, screenPos.y, menuNodes[i].x , menuNodes[i].y ) ;
    ...
    if ( closestDistance < menuNodes[closestIndex].width * forwardTouchMinRatio )
    {

        if ( isTouchIdle == false )
        {
            int selectionIndex = menuNodes[closestIndex].forwardedTouchDown( tcur.getX(), tcur.getY(), tcur.getCursorID(), &multiTouchCustomData ) ;
            firstTouch = false ;
        ...
        if ( isTouchIdle == true )
        {
            cout << " currently idle! menu " << closestIndex << " was hit!" << endl ;
            menuNodes[closestIndex].forwardedTouchUp( tcur.getX(), tcur.getY(), tcur.getCursorID() ) ;
            menuNodes[closestIndex].changeState ( 1 ) ;
            for ( int a = 0 ; a < nItems ; a++ )
            {
                if ( a != closestIndex )
                    menuNodes[a].changeState( 0 ) ;
            }
            toggleIdle() ;
            lastTouchTime = ofGetElapsedTimef() ;
            isTouchIdle = false ;
...
void testApp::tuioCursorRemoved(TuioCursor & tcur)
{
        ...
        for ( int i = 0 ; i < nItems ; i++ )
        {
           // cout << "item @ " << i << " state = " << menuNodes[i].state << endl ;
            float dist = ofDist(screenPos.x, screenPos.y, menuNodes[i].x , menuNodes[i].y ) ;

            if ( dist < menuNodes[i].width * forwardTouchMinRatio )
            {
                if ( menuNodes[i].state == 0 || menuNodes[i].state == 2 )
                {
                    menuNodes[i].playSubNode = false ;
                    cout << "should be touching up!" <<endl ;
                    menuNodes[i].forwardedTouchUp( tcur.getX(), tcur.getY(), tcur.getCursorID() ) ;

                    for ( int a = 0 ; a < nItems ; a++ )
                    {
                        if ( a != i && menuNodes[a].state == 3 )
                        {
                            menuNodes[a].nextState = 0 ;
                            menuNodes[a].changeState( 0 ) ;
                        }
                    }
                }
                // && menuNodes[i].inFlickTransition == false
                //else if ( menuNodes[i].inBoundsTransition == false )
                //{
                  if ( doFlick == true && menuNodes[i].inBoundsTransition == false )
                  {
